...
void JSON::checkString(std::string& s)
{
...
std::any JSON::string2any(std::string& s)
{
...
std::map<std::string, std::any> JSON::parseString(std::string& s)
{
...
std::map<std::string, std::string> parseFromFile(std::string fileName)
{
	std::map<std::string, std::any> sourceMap = JSON::parse(fileName);
	std::map<std::string, std::string> returnMap;
	for (auto const& pair : sourceMap)
	{
		returnMap.insert(std::make_pair(pair.first, std::any_cast<std::string>(pair.second)));
	}
	return returnMap;
}
